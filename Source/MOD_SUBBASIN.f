      ...
      TYPE SUBBASIN_TYPE
        ...
        PROCEDURE,PASS(SEFT), PUBLIC :: SBSRESET_MEMORIES
     ...
     &           (14104.0D0*SEFT%CN**0.70D0*SEFT%SLOPE**0.50D0)
        ...
        SUBROUTINE SBSRESET_MEMORIES(SEFT)
        CLASS(SUBBASIN_TYPE), INTENT(INOUT) :: SEFT
        INTEGER :: I, IDX
        ...
        DO I = 1, NFCT*2
            ...
            SEFT%LOSS(I) = SEFT%LOSS(IDX)
            SEFT%EXCESS(I) = SEFT%EXCESS(IDX)
            SEFT%DIRECT_FLOW(I) = SEFT%DIRECT_FLOW(IDX)
            SEFT%TOTAL_FLOW(I) = SEFT%TOTAL_FLOW(IDX)
            SEFT%BASE_FLOW(I) = SEFT%BASE_FLOW(IDX)
            SEFT%AVGPRECIP(I) = SEFT%AVGPRECIP(IDX)
            IDX = IDX + 1

        ENDDO

        SEFT%LOSS(NFCT*2 + 1:) = 0.0D0
        SEFT%EXCESS(NFCT*2 + 1:) = 0.0D0
        SEFT%DIRECT_FLOW(NFCT*2 + 1:) = 0.0D0
        SEFT%BASE_FLOW(NFCT*2 + 1:) = 0.0D0
        SEFT%TOTAL_FLOW(NFCT*2 + 1:) = 0.0D0
        SEFT%AVGPRECIP(NFCT*2 + 1:) = 0.0D0

        END SUBROUTINE SBSRESET_MEMORIES
